  ...
  const { data, error } = await supabase
    ...
    .select(`
      *,
      ...
      client_file_vouchers ( id, voucher_name, voucher_type, voucher_number, file_path, pdf_url, form_data, issued_by_name, uploaded_at, updated_at ),
...
// Updates an existing draft's fields without generating a PDF — used when editing a
// previously-saved draft before finally generating it.
export async function updateVoucherDraft(voucherId, formData, issuedByName) {
  const { data, error } = await supabase
    .from('client_file_vouchers')
    .update({ form_data: formData, issued_by_name: issuedByName || null })
    .eq('id', voucherId)
    .select()
    .single()
  if (error) throw error
...
// Pass voucherId to regenerate an existing voucher in place (reusing its number and
// overwriting its PDF) instead of creating a new one.
export async function generateVoucher(clientFileId, voucherType, formData, voucherId) {
  const { data, error } = await supabase.functions.invoke('generate-voucher-pdf', {
    body: { clientFileId, voucherType, formData, voucherId: voucherId || undefined },
  })
  if (error) throw error
